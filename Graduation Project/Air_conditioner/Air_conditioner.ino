#include <Arduino.h>
#include <ESP8266WiFi.h>
#include <PubSubClient.h>
#include <IRac.h>
#include <IRutils.h>
#include <IRremoteESP8266.h>
#include "Base64.h"
#include <NTPClient.h>
#include <WiFiUdp.h>

/*************************************** Config ********************************************************/
WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP, "ntp.aliyun.com"); //NTP地址
WiFiClient     espClient;                             //创建一个 WiFi 客户端
PubSubClient   mqttClient;                            //创建一个 mqtt 客户端
//OneNet MQTT 配置
const char*    mqttServer = "183.230.40.39";          //目前 Onenet mqtt服务器 的 ip地址
const uint16_t mqttPort = 6002;                       //目前 Onenet mqtt服务器 的 端口号
int wSpeed = 0;
int Modec = 0;
int Temp = 25;
int LRs = 0;
int UDs = 0;
/************************************ 请修改以下区域 *****************************************************/
#define PRODUCT_ID    "012345"                        //产品ID，需要更改为你自己创建的产品的 ID     
#define API_KEY       "0123456789012345678901234567"  //APIKEY，需要更改为你自己创建的产品的 APIKEY
#define DEVICE_ID     "012345678"                     //设备ID，需要更改为你自己创建的产品的 设备ID

//WiFi和空调 配置
const char* ssid     = "0123456";                       //WiFi账号：需要修改为所处环境的WiFi 账号
const char* password = "0123456";                 //WiFi密码：秘密需要修改为所处环境的WiFi 密码

String mykey = "0123456789012345";                    //设定16字节字符串，与APP端保持一致
String birth = "0123456";                             //设定7字节字符串，与APP端保持一致
int firm = 24;                                         //空调品牌（具体见文档）
const uint16_t kIrLed = D2;                           //设置红外发射数据引脚为D2
IRac ac(kIrLed);
/******************************************************************************************************/

// AES CBC Decryption
String decrypt(String encoded_data_str, String myiv) {
  uint8_t my_key[16];
  uint8_t my_iv[16];
  for (int j = 0; j < 16; j++) {
    my_key[j] = mykey.charAt(j);
  }
  for (int k = 0; k < 16; k++) {
    my_iv[k] = myiv.charAt(k);
  }
  int input_len = encoded_data_str.length();
  char *encoded_data = const_cast<char*>(encoded_data_str.c_str());
  int len = base64_dec_len(encoded_data, input_len);
  uint8_t data[ len ];
  base64_decode((char *)data, encoded_data, input_len);

  uint8_t key[16], iv[16];
  memcpy(key, my_key, 16);
  memcpy(iv, my_iv, 16);

  int n_blocks = len / 16;

  br_aes_big_cbcdec_keys decCtx;

  br_aes_big_cbcdec_init(&decCtx, key, 16);
  br_aes_big_cbcdec_run( &decCtx, iv, data, n_blocks * 16 );

  // PKCS#7 Padding (Decryption)
  uint8_t n_padding = data[n_blocks * 16 - 1];
  len = n_blocks * 16 - n_padding;
  char plain_data[len + 1];
  memcpy(plain_data, data, len);
  plain_data[len] = '\0';

  return String(plain_data);
}

//Onenet MQTT 连接
void Onenet_MQTT_Connect() {
  int cnt = 0;
  while (!mqttClient.connected()) {
    cnt++;
    Serial.println("Connect to OneNet MQTT...");
    /* 连接 Onenet 云平台*/
    if (mqttClient.connect(DEVICE_ID, PRODUCT_ID, API_KEY)) {
      digitalWrite(LED_BUILTIN, LOW);
      Serial.println("\r\nconnect success!");
      return ;
    } else {
      digitalWrite(LED_BUILTIN, HIGH);
      Serial.print("Connect Fail!");
      Serial.println("Retry in 1 seconds");
      delay(1000);
    }
    if (cnt >= 10) {
      cnt = 0;
      Serial.println("Connet Failed");
      Serial.println("Please Check API_KEY and  Device ID,Product ID");
    }
  }
  return ;
}

//MQTT 消息回调函数
void callback(char* topic, byte* payload, unsigned int length) {
  int i ;
  String Open = "1";
  String Close = "0";
  String Mode = "2";
  String Wind = "3";
  String Hup = "4";
  String Cdown = "5";
  String LRswing = "6";
  String UDswing = "7";
  char encdata[30];
  timeClient.update();
  unsigned long epochTime__5 = timeClient.getEpochTime() / 5;
  String epochTime_5__1 = (String)(epochTime__5 + 1);
  String epochTime_5__2 = (String)(epochTime__5 + 2);
  String epochTime_5 = (String)(epochTime__5 );
  String epochTime_5_1 = (String)(epochTime__5 - 1);
  String epochTime_5_2 = (String)(epochTime__5 - 2);

  String myiv__1 = birth + epochTime_5__1;
  String myiv__2 = birth + epochTime_5__2;
  String myiv = birth + epochTime_5;
  String myiv_1 = birth + epochTime_5_1;
  String myiv_2 = birth + epochTime_5_2;
  decode_type_t protocol = (decode_type_t)firm;
  if (ac.isProtocolSupported(protocol)) {
    Serial.println("Protocol " + String(protocol) + " / " +
                   typeToString(protocol) + " is supported.");
    ac.next.protocol = protocol;  // Change the protocol used.
    Serial.print("Message Recive: topic[");
    Serial.print(topic);
    Serial.print("],value: ");
    for ( i = 0; i < length; i++) {
      encdata[i] = (char)payload[i];
    }
    encdata[i++] = '\0';
    Serial.println(encdata);

    String decdata__1 = decrypt(encdata, myiv__1);
    String decdata__2 = decrypt(encdata, myiv__2);
    String decdata = decrypt(encdata, myiv);
    String decdata_1 = decrypt(encdata, myiv_1);
    String decdata_2 = decrypt(encdata, myiv_2);

    if (decdata__1 == Open || decdata__2 == Open || decdata == Open || decdata_1 == Open || decdata_2 == Open) {

      ac.next.power = true;  // We want to turn on the A/C unit.
      ac.sendAc();  // Have the IRac class create and send a message.
      Serial.println("open");
    }
    else if (decdata__1 == Close || decdata__2 == Close || decdata == Close || decdata_1 == Close || decdata_2 == Close) {

      ac.next.power = false;  // Now we want to turn the A/C off.
      ac.sendAc();  // Send the message.
      Serial.println("close");
    }
    else if (decdata__1 == Mode || decdata__2 == Mode || decdata == Mode || decdata_1 == Mode || decdata_2 == Mode) {
      if (Modec == 0) {
        ac.next.mode = stdAc::opmode_t::kDry;  // Now we want to set the mode.
        ac.sendAc();  // Send the message.
        Serial.println("mode change to dry");
        Modec++;
      }
      else if (Modec == 1) {
        ac.next.mode = stdAc::opmode_t::kFan;  // Now we want to set the mode.
        ac.sendAc();  // Send the message.
        Serial.println("mode change to fan");
        Modec++;
      }
      else if (Modec == 2) {
        ac.next.mode = stdAc::opmode_t::kHeat;  // Now we want to set the mode.
        ac.sendAc();  // Send the message.
        Serial.println("mode change to heat");
        Modec++;
      }
      else if (Modec == 3) {
        ac.next.mode = stdAc::opmode_t::kAuto;  // Now we want to set the mode.
        ac.sendAc();  // Send the message.
        Serial.println("mode change to auto");
        Modec++;
      }
      else if (Modec == 4) {
        ac.next.mode = stdAc::opmode_t::kCool;  // Now we want to set the mode.
        ac.sendAc();  // Send the message.
        Serial.println("mode change to cool");
        Modec = 0;
      }
    }
    else if (decdata__1 == Wind || decdata__2 == Wind || decdata == Wind || decdata_1 == Wind || decdata_2 == Wind) {
      if (wSpeed == 0) {
        ac.next.fanspeed = stdAc::fanspeed_t::kLow;  // Now we want to set the wSpeed.
        ac.sendAc();  // Send the message.
        Serial.println("wind speed change to low");
        wSpeed++;
      }
      else if (wSpeed == 1) {
        ac.next.fanspeed = stdAc::fanspeed_t::kMedium;  // Now we want to set the wSpeed.
        ac.sendAc();  // Send the message.
        Serial.println("wind speed change to medium");
        wSpeed++;
      }
      else if (wSpeed == 2) {
        ac.next.fanspeed = stdAc::fanspeed_t::kHigh;  // Now we want to set the wSpeed.
        ac.sendAc();  // Send the message.
        Serial.println("wind speed change to high");
        wSpeed++;
      }
      else if (wSpeed == 3) {
        ac.next.fanspeed = stdAc::fanspeed_t::kAuto;  // Now we want to set the wSpeed.
        ac.sendAc();  // Send the message.
        Serial.println("wind speed change to auto");
        wSpeed = 0;
      }
    }
    else if (decdata__1 == Hup || decdata__2 == Hup || decdata == Hup || decdata_1 == Hup || decdata_2 == Hup) {
      if (Temp < 30) {
        ac.next.degrees = ++Temp;  // Now we want to turn the degree up.
        ac.sendAc();  // Send the message.
        Serial.println("heat up to " + (String)Temp);
      }
      else {
        Serial.println("It has been heated to the maximum !");
      }
    }
    else if (decdata__1 == Cdown || decdata__2 == Cdown || decdata == Cdown || decdata_1 == Cdown || decdata_2 == Cdown) {
      if (Temp > 18) {
        ac.next.degrees = --Temp;  // Now we want to turn the degree down.
        ac.sendAc();  // Send the message.
        Serial.println("cool down to " + (String)Temp);
      }
      else {
        Serial.println("It has been cooled to the minimum !");
      }
    }
    else if (decdata__1 == LRswing || decdata__2 == LRswing || decdata == LRswing || decdata_1 == LRswing || decdata_2 == LRswing) {
      if (LRs == 0) {
        ac.next.swingh = stdAc::swingh_t::kAuto;  // Swing the fan left and right.
        ac.sendAc();  // Send the message.
        Serial.println("left and right swing on");
        LRs++;
      }
      else {
        ac.next.swingh = stdAc::swingh_t::kOff;  // Don't swing the fan left or right.
        ac.sendAc();  // Send the message.
        Serial.println("left and right swing off");
        LRs = 0;
      }
    }
    else if (decdata__1 == UDswing || decdata__2 == UDswing || decdata == UDswing || decdata_1 == UDswing || decdata_2 == UDswing) {
      if (UDs == 0) {
        ac.next.swingv = stdAc::swingv_t::kAuto;  // Swing the fan up and down.
        ac.sendAc();  // Send the message.
        Serial.println("up and down swing on");
        UDs++;
      }
      else {
        ac.next.swingv = stdAc::swingv_t::kOff;  // Don't swing the fan up or down.
        ac.sendAc();  // Send the message.
        Serial.println("up and down swing off");
        UDs = 0;
      }
    }
  }
}
void setup() {
  // AC Common 配置
  // Set up what we want to send.
  // See state_t, opmode_t, fanspeed_t, swingv_t, & swingh_t in IRsend.h for
  // all the various options.
  ac.next.model = 1;  // Some A/Cs have different models. Try just the first.
  ac.next.mode = stdAc::opmode_t::kCool;  // Run in cool mode initially.
  ac.next.celsius = true;  // Use Celsius for temp units. False = Fahrenheit
  ac.next.degrees = 25;  //默认温度
  ac.next.fanspeed = stdAc::fanspeed_t::kAuto;  // Start the fan at medium.
  ac.next.swingv = stdAc::swingv_t::kOff;  // Don't swing the fan up or down.
  ac.next.swingh = stdAc::swingh_t::kOff;  // Don't swing the fan left or right.
  ac.next.light = true;  // Turn on any LED/Lights/Display that we can.
  ac.next.beep = false;  // Turn off any beep from the A/C if we can.
  ac.next.econo = false;  // Turn off any economy modes if we can.
  ac.next.filter = false;  // Turn off any Ion/Mold/Health filters if we can.
  ac.next.turbo = false;  // Don't use any turbo/powerful/etc modes.
  ac.next.quiet = false;  // Don't use any quiet/silent/etc modes.
  ac.next.sleep = -1;  // Don't set any sleep time or modes.
  ac.next.clean = false;  // Turn off any Cleaning options if we can.
  ac.next.clock = -1;  // Don't set any current time if we can avoid it.
  ac.next.power = false;  // Initially start with the unit off.
  //Start Serial
  //#0. 配置串口
  Serial.begin (115200);
  Serial.println("\r\n\r\nESP826(MQTT)Start:  ");
  delay(100);
  //#1. 连接WIFI
  WiFi.begin(ssid, password);//连接到WiFi
  Serial.print("Wait for WiFi Connect");
  int wait_cnt = 0;
  while (WiFi.status() != WL_CONNECTED) {
    if (50 <= wait_cnt++) {
      wait_cnt = 0;
      Serial.println("\r\nConnect Fialed");
      Serial.println("Please Check Your WiFi SSID and PWD or");
      Serial.println("Reset the Device and Retry");
    }
    delay(500);
    Serial.print(".");
  }
  timeClient.begin();
  timeClient.setTimeOffset(28800); //+1区，偏移3600，+8区，偏移3600*8
  Serial.println(" ");

  pinMode(LED_BUILTIN, OUTPUT);

  //#2. 客户端设置MQTT服务器 & 回调函数
  mqttClient.setServer(mqttServer, mqttPort);
  mqttClient.setClient(espClient);
  mqttClient.setCallback(callback);
}

void loop() {
  //#3.连接 Onenet
  Onenet_MQTT_Connect();
  mqttClient.loop();
}
