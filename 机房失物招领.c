...
void read1()//读取已领文本
{
	...
	while(1)
	{
		...
		printf("\n\n姓名:%s\t物品:%s\t种类:%s\t学号:%s\t手机号:%s\t丢失地点:%s\t认领状态:%s\t\t%s",student.name,student.thing,student.zhonglei,student.xuehao,student.shoujihao,student.didian,student.zhuangtai,student.time);
	}
	fclose(fp);
}
		...
		while(1)
	{
		...
		else if(strcmp(b,x)==0)
		{
			...
			if(i)
			{
				...
				fp2 = fopen(shijian,"a");
				fprintf(fp2,"姓名:%s\t物品:%s\t种类:%s\t学号:%s\t手机号:%s\t丢失地点:%s\t状态:%s\t时间:%s\n",student.name,student.thing,student.zhonglei,student.xuehao,student.shoujihao,student.didian,student.zhuangtai,student.time);
